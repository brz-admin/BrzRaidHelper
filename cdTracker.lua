...
function tracker.getMyCds()
	...
	while true do
		...
		local cd = util.getSpellCooldownInBook(i)
		-- only spells that have more than 30s CD are interresting here
		if (cd > 30) then
			...
			if (spellString ~= "") then spellString = spellString .. ';' end
			local myspell = spellName .. '::' .. up;
			spellString = spellString .. myspell
	...
	for bag = 0, 4 do
		if GetContainerNumSlots(bag) > 0 then
			for slot = 0, GetContainerNumSlots(bag) do
				local itemName, secondLine = util.getBagItemName(bag, slot)
				-- Only send CD for potions and engineering items
				if (itemName ~= nil and (string.find(strlow(itemName), 'potion') or (secondLine ~= nil and string.find(strlow(secondLine), 'requi')))) then
	...
	util.addonCom("myCds", spellString)
end
...
tracker.nextSpellTick = GetTime() + 0.2; -- every 0.2 seconds we check
...
BRH_CDTracker.main:SetScript("OnUpdate", function() 
	...
	if (tracker.nextSpellTick and tracker.nextSpellTick <= GetTime()) then
		for playerName, spellList in pairs(tracker.cdListQueue) do
			if (not spellList.done) then
				local thisPlayerSpells = util.strsplit(';', spellList.spellString);
				for idx, spellBlock in ipairs(thisPlayerSpells) do
					local spellData = util.strsplit("::", spellBlock)
		...
		tracker.nextSpellTick = GetTime() + 0.2;
	end
end)
...
function tracker.HandleAddonMSG(sender, data)
	...
	elseif (cmd == "getTrackedSpells") then
		tracker.getMyCds()
	elseif (cmd == "myCds") then
		if (tracker.cdListQueue[sender] == nil) then
			tracker.cdListQueue[sender] = {
				["done"] = false,
				["spellString"] = datas
			}
		end
